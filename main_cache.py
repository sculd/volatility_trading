...
def get_cache_df_daily_history(ticker, date_from_str = "2020-01-01", date_to_str = "2024-07-02"):
...
def get_cache_df_intraday_history(ticker):
...
def get_cache_df_market_open_close_history(ticker):
    df_intraday_history = data.intraday.load_df_intraday_history(ticker)
    df_market_open_close_history = data.daily.get_df_market_open_close_history_from_intraday_history(df_intraday_history)
    df_name = f"df_{ticker}_market_open_close_history"
    df_market_open_close_history.to_pickle(f'market_data/{df_name}.pkl')
...
def cache_ticker(ticker):
    ...
    df_daily_history = get_cache_df_daily_history(ticker)
    print(f"df_daily_history:\n{df_daily_history}")

    df_intraday_history = get_cache_df_intraday_history(ticker)
    print(f"df_intraday_history:\n{df_intraday_history}")

    df_market_open_close_history = get_cache_df_market_open_close_history(ticker)
    print(f"df_market_open_close_history:\n{df_market_open_close_history}")
